...
bool disk_read_auto(uint32_t lba, uint64_t target_ram_addr) {
    ...
    if (selected_drive_idx == -1) {
        ...
        if (f) {
            fread((void*)target_ram_addr, 1, 1024*1024, f); /// Read up to 1 MB into target RAM
            fclose(f);
        }
        return true;
    }
...
_50 debug_print(const char* msg) {
    _15(!windows[7].open) {
        ...
        str_cpy(windows[7].title, "SYSTEM DEBUG (POPUP)");
        windows[7].x = 200; windows[7].y = 10;
        windows[7].w = 400; windows[7].h = 200;
...
#else
    while(1) { } // In Linux, just spin or exit
#endif
}
...
uint64_t resolve_windows_api(const char* dll_name, const char* func_name) {
    if (!func_name || !dll_name) return 0;
    
    char u_dll[64] = {0};
    for(int i=0; i<63 && dll_name[i]; i++) {
        u_dll[i] = dll_name[i];
        if (u_dll[i] >= 'a' && u_dll[i] <= 'z') u_dll[i] -= 32;
    }
    
    Window* w = &windows[2];
    char msg[128];
    snprintf(msg, 128, "[WINE] IMPORT: %s -> %s\n", u_dll, func_name);
    print_win(w, msg);
    
    auto my_strcmp = [](const char* a, const char* b) {
        while(*a && *a == *b) { a++; b++; }
        return (*a == *b);
    };
    
    if (my_strcmp(u_dll, "KERNEL32.DLL") || my_strcmp(u_dll, "KERNEL32")) {
        if (my_strcmp(func_name, "ExitProcess")) return (uint64_t)&Mock_ExitProcess;
    } else if (my_strcmp(u_dll, "USER32.DLL") || my_strcmp(u_dll, "USER32")) {
        if (my_strcmp(func_name, "MessageBoxA")) return (uint64_t)&Mock_MessageBoxA;
    }
...
extern "C" void kernel_main64(BootInfo* boot_info) {
    ...
    str_cpy(windows[0].title, "NOTEPAD");  windows[0].x=0; windows[0].y=0; windows[0].w=750; windows[0].h=300; windows[0].color=0xEEEEEE; 
    str_cpy(windows[1].title, "APPS");     windows[1].x=0; windows[1].y=300; windows[1].w=750; windows[1].h=400; windows[1].color=0xDDDDDD; 
    str_cpy(windows[2].title, "SAVE AS..."); windows[2].x=0; windows[2].y=150; windows[2].w=300; windows[2].h=200; windows[2].color=0xDDDDDD;
    str_cpy(windows[3].title, "SYSTEM");   windows[3].x=0; windows[3].y=0;  windows[3].w=750; windows[3].h=480; windows[3].color=0xFFD700;
    str_cpy(windows[4].title, "DISK MGR"); windows[4].x=0; windows[4].y=0; windows[4].w=750; windows[4].h=400; windows[4].color=0x888888; 
    str_cpy(windows[5].title, "CMD");      windows[5].x=0; windows[5].y=0; windows[5].w=750; windows[5].h=400; windows[5].color=0x111111;
    str_cpy(windows[6].title, "EMULATOR"); windows[6].x=0; windows[6].y=0; windows[6].w=750; windows[6].h=400; windows[6].color=0x222222;
    ...
    str_cpy(windows[9].title, "IMAGE VIEWER"); windows[9].x=0; windows[9].y=0; windows[9].w=750; windows[9].h=400; windows[9].color=0x111111;
    str_cpy(windows[10].title, "GERAETE MGR"); windows[10].x=100; windows[10].y=80; windows[10].w=750; windows[10].h=400; windows[10].color=0x111111;
    str_cpy(windows[11].title, "ARCADE: PONG"); windows[11].x=200; windows[11].y=150; windows[11].w=750; windows[11].h=400; windows[11].color=0x112211;
    str_cpy(windows[12].title, "ARCADE: BLOBBY"); windows[12].x=250; windows[12].y=100; windows[12].w=750; windows[12].h=400; windows[12].color=0x4488FF;
	str_cpy(windows[14].title, "SMASH CATS 3D"); windows[14].x=250; windows[14].y=100; windows[14].w=750; windows[14].h=400; windows[14].color=0x4488FF;
    ...
    _114(1) {
        ...
        if (key_ready) {
            ...
            _15(!(sc & 0x80)) { /// Taste wurde GEDRÜCKT (Make Code)
				...
				_15(sc EQ 0x40) toggle_app(6); /// F6 = Emulator
		...
		app_window_active = _86; // Setzt die 3D Engine in jedem Frame standardmäßig auf AUS (_86 = false)
        _39(_43 i=0; i<200; i++) {
            _43 t = stars[i].type;
        ...
        _39(_43 i=0; i<5; i++) {
            ...
            _15(galaxy_expansion > 10 OR win->minimized OR win->open) {
                ...
                _15(!z_blocked AND mouse_just_pressed AND !click_consumed AND hov) { 
                    ...
                    _41 { win->open = _128; focus_window(win->id); }
                    click_consumed = _128;
                }
            }
        }
        ...
        _39(_43 i=0; i<20; i++) {
                ...
                _15(win->content[0] EQ 'B' AND win->content[1] EQ 'M' AND win->content[2] EQ 'P') {
                    ...
                    _15(bmp[0] EQ 'B' AND bmp[1] EQ 'M') {
                        ...
                        _15(bpp EQ 24 AND img_w > 0 AND img_h > 0 AND img_w < 1920 AND img_h < 1080) {
                            ...
                            TextC(wx + ww/2, wy + wh/2, "UNSUPPORTED BMP (24-BIT ONLY)", 0xFF0000, _128);
                        }
                    } _41 {
                        TextC(wx + ww/2, wy + wh/2, "INVALID BMP FILE", 0xFF0000, _128);
                    }
                }
            }
            ...
            _15(win->id EQ 3) {
				...
				#ifdef __linux__
				// ==========================================
                // FEATURE 1: UPDATE BUTTON (Oben Rechts)
                // ==========================================
                static int update_state = 0; 
                static char update_status[64] = "UPDATE OS";
                
                // Button Farbe ändert sich bei Erfolg auf Grün (0x00AA00)
                DrawRoundedRect(wx + ww - 130, wy + 40, 110, 25, 4, (update_state == 2) ? 0x00AA00 : ((update_state == 1) ? 0xAA8800 : 0x0055AA));
                TextC(wx + ww - 75, wy + 48, update_status, 0xFFFFFF, _86);
                
                if (input_cooldown == 0 && mouse_just_pressed && is_over_rect(mouse_x, mouse_y, wx + ww - 130, wy + 40, 110, 25)) {
                    str_cpy(update_status, "SEARCHING...");
                    update_state = 1;
                    
                    // WSL-FIX: Sucht nicht in /mnt/c (Windows-Laufwerk) und unterdrückt alle Fehler (2>/dev/null)
                    int ret = ::system("find /media /mnt -maxdepth 3 -path '/mnt/c' -prune -o -type f -name 'meinos.elf' -print 2>/dev/null | head -n 1 | grep -q . && find /media /mnt -maxdepth 3 -path '/mnt/c' -prune -o -type f -name 'meinos.elf' -print 2>/dev/null | head -n 1 | xargs -I {} sudo cp -f {} /opt/meinos/meinos.elf");
                    
                    if (ret == 0) {
                        str_cpy(update_status, "UPDATED! REBOOT");
                        update_state = 2;
                    } else {
                        str_cpy(update_status, "NOT FOUND!");
                        update_state = 0;
                    }
                    input_cooldown = 30;
                }
                ...
                // ==========================================
                // FEATURE 2: INSTALL TO DISK BUTTON (Unter dem Update Button)
                // ==========================================
                int btn_inst_w = 180;
                int btn_inst_x = wx + ww - btn_inst_w - 20; 
                int btn_inst_y = wy + 75; 
                
                DrawRoundedRect(btn_inst_x, btn_inst_y, btn_inst_w, 25, 4, 0xAA0000);
                TextC(btn_inst_x + (btn_inst_w / 2), btn_inst_y + 8, "INSTALL TO HARD DISK", 0xFFFFFF, _86);
                ...
                if (input_cooldown == 0 && mouse_just_pressed && is_over_rect(mouse_x, mouse_y, btn_inst_x, btn_inst_y, btn_inst_w, 25)) {
                    ...
                    ::system("xterm -hold -bg black -fg '#00FF00' -fa 'Monospace' -fs 11 -title 'CosmOS Installer' -e 'sudo bash ./install_cosmos.sh' &");
            ...
            _15(win->id EQ 1) { 
                ...
                TextC(col1+bw/2, row1+10, "SMASH CATS 3D", 0xFFFFFF, _128);
                
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, col1, row1, bw, bh)) {
                    windows[10].open = _128; windows[10].minimized = _86; str_cpy(windows[10].title, "SMASH CATS 3D"); focus_window(10);
                ...
                TextC(col2+bw/2, row2+10, "COSMOS DEFENDER", 0xFFFFFF, _128);
                
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, col2, row2, bw, bh)) {
                    windows[14].open = _128; windows[14].minimized = _86; str_cpy(windows[14].title, "COSMOS DEFENDER"); focus_window(14);
                    input_cooldown = 25; windows[1].open = _86; windows[2].open = _86;
                }
                ...
                DrawRoundedRect(col1, row3, bw, bh, 4, 0xCC5500); 
                ...
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, col1, row3, bw, bh)) { 
                    ...
                    if (pid == 0) {
                        // Das hier ist der "Kind-Prozess" (Das neue Programm)
                        // execlp ersetzt das Kind komplett durch den Browser
                        execlp("netsurf-gtk", "netsurf-gtk", NULL);
                        exit(1); // Falls es schiefgeht, Kind beenden
                    } else if (pid > 0) {
                        // Das hier ist die "Mutter" (Deine CosmOS Engine)
                        // Wir haben jetzt die 'pid' (z.B. 4012)! 
                        
                        // HIER trägst du die PID in dein TSK Array ein!
                        // z.B. AddTaskToScheduler(pid, "Web Browser");
                    }
#endif
                    input_cooldown = 25; windows[1].open = _86; windows[2].open = _86; 
                }
                ...
                DrawRoundedRect(col2, row3, bw, bh, 4, 0xCC5500);
                ...
                DrawRoundedRect(col3, row3, bw, bh, 4, 0xCC5500);
				...
				/// --- ANTIGRAVITY KI ASSISTANT ---
                int row4 = row3 + 60; // <--- HIER WIRD ROW4 BERECHNET (Passe die 60 an, falls der Abstand nicht stimmt)
                
                DrawRoundedRect(col1, row4, bw, bh, 4, 0xFF9900); // Oranger AI-Button
                TextC(col1+bw/2, row4+10, "KI ASSISTANT", 0xFFFFFF, _128); 
                
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, col1, row4, bw, bh)) { 
#ifdef __linux__
                    // Der Startbefehl für die Antigravity-App
				...
				/// --- DISK MANAGER (PCMANFM) ---
                DrawRoundedRect(col2, row3, bw, bh, 4, 0xCC5500); 
                TextC(col2+bw/2, row3+10, "FILE MANAGER", 0xFFFFFF, _128); 
                
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, col2, row3, bw, bh)) { 
#ifdef __linux__
                    // Das öffnet das echte Tool als eigenes Fenster. Kein Terminal, kein Text-Gedöns.
                    ::system("thunar &");
#endif
                    input_cooldown = 25; 
                }
                            ...
                            _15(!is_ntfs_drive AND !is_fat32_drive) {
                                ...
                                _15(free_slot != -1) {
                                    ...
                                    _15(!clipboard_is_folder) {
                                        _39(int s=0; s<10; s++) {
                                            ahci_write_sectors(target_port, target_lba + s, 1, (uint64_t)global_buf_dir + (s * 512));
                                            _39(_192 _43 wait = 0; wait < 50000; wait++) ;
                                        }
                        ...
                        _15(cfs_files[i].exists AND cfs_files[i].parent_idx EQ current_folder_id) {
                            ...
                            _15(input_cooldown EQ 0 AND mouse_just_pressed AND is_over_rect(mouse_x, mouse_y, wx+190, y_off, 40, 16)) {
                                ...
                                _15(cfs_files[i].is_folder) {
                                    ...
                                    _44 is_tba = _86; _44 is_jpg_png = _86; _44 is_bmp = _86; _44 is_ch8 = _86; _44 is_bin = _86; _44 is_wav = _86;
            ...
            _15(win->id EQ 7) {
                ...
                Text(wx+15, wy+75, fps_buf, 0x00FF00, _128);
            }
        ...
        uint32_t diff = system_ticks - last_frame_ticks;
        if (diff < 4) {
            usleep((4 - diff) * 1000);
        }
        last_frame_ticks = system_ticks;
#endif
        frame++;
    }
}
